...
class WGenerator(object):
    ...
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
        ...
        self.x = tf.compat.v1.placeholder(  # true data, not including start token
            tf.int32, shape=[self.batch_size, self.sequence_length])
        self.rewards = tf.compat.v1.placeholder(  # rom rollout policy and discriminator
        ...
        with tf.name_scope('gen_training'):
            self.g_loss = -tf.reduce_sum(
                tf.reduce_sum(
                    tf.one_hot(tf.compat.v1.to_int32(tf.reshape(self.x, [-1])), self.num_emb, 1.0, 0.0) *
                    tf.compat.v1.log(
